...
with tabs[0]:
    ...
    cap_long = capacity_df.reset_index().melt(var_name="Model", value_name="Max Units")
    fig = px.bar(cap_long, x="Model", y="Max Units", text="Max Units")
    fig.update_layout(template="plotly_white")
    st.plotly_chart(fig, use_container_width=True, key="dashboard_capacity_chart")

    # ---------------- Tables ----------------
    st.subheader("⚠️ Bottlenecks")
    st.dataframe(make_df_arrow_safe(bottlenecks_df), use_container_width=True)

    st.subheader("🏭 Supplier Risk")
    st.dataframe(make_df_arrow_safe(supplier_df), use_container_width=True)
...
with tabs[1]:
    ...
    user_input = st.chat_input("Ask about capacity, bottlenecks, suppliers, demand scenarios...")
...
with tabs[4]:
    ...
    spike = st.slider("Increase demand by (%)", min_value=0, max_value=100, value=20)
    ...
    if st.button("Run Simulation"):
        ...
        st.info("Capacity represents immediate build potential. Demand spikes affect risk over time, not instant capacity.")
        sim_capacity_df = pd.DataFrame(sim_capacity)
        cap_long = sim_capacity_df.reset_index().melt(var_name="Model", value_name="Max Units")
        fig_cap = px.bar(cap_long, x="Model", y="Max Units", text="Max Units", title="Immediate Build Capacity")
        fig_cap.update_layout(template="plotly_white")
        st.plotly_chart(fig_cap, use_container_width=True, key=f"sim_capacity_chart_{spike}")

        # ---------------- Days of Cover ----------------
        st.subheader("🔥 Inventory Risk After Demand Spike")
        sim_parts_df = pd.DataFrame(sim_snapshot)
        doc_df = sim_parts_df[["part_id", "days_of_cover"]].sort_values("days_of_cover")
        fig_doc = px.bar(doc_df, x="part_id", y="days_of_cover", text="days_of_cover", title="Days of Cover After Demand Spike")
        fig_doc.update_layout(template="plotly_white")
        st.plotly_chart(fig_doc, use_container_width=True, key=f"days_of_cover_chart_{spike}")
        st.caption("Lower days of cover indicate higher stockout risk as demand increases.")
        ...
        if sim_bottlenecks_df.empty:
            ...
            st.dataframe(make_df_arrow_safe(sim_bottlenecks_df), use_container_width=True)
